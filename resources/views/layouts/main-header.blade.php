<header class="header header-wrapper">
    <nav class="container header-navigation">
        <a href="" class="top">
            <svg width="95" height="51" viewBox="0 0 95 51" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path
                    d="M40.902 7.44849C32.0326 7.44849 24.8223 14.515 24.8223 23.2368C24.8223 31.9585 32.0326 39.0251 40.902 39.0251C49.7714 39.0251 56.9818 31.9585 56.9818 23.2368C56.9818 14.515 49.7714 7.44849 40.902 7.44849ZM40.902 34.2504C34.7126 34.2504 29.6717 29.3484 29.6717 23.2368C29.6717 17.1252 34.7126 12.2232 40.902 12.2232C47.0914 12.2232 52.1323 17.1252 52.1323 23.2368C52.1323 29.3484 47.0914 34.2504 40.902 34.2504Z"
                    fill="white" />
                <path
                    d="M78.9947 7.44849C74.3367 7.44849 70.1891 9.61301 67.3815 13.0508V8.2761H62.2769V50.8026H67.3815V33.4228C70.1891 36.8605 74.3367 39.0251 78.9947 39.0251C87.4174 39.0251 94.1811 31.9585 94.1811 23.2368C94.1811 14.515 87.3536 7.44849 78.9947 7.44849ZM78.3566 34.314C72.4224 34.314 67.6368 29.3484 67.6368 23.2368C67.6368 17.1252 72.4224 12.1595 78.3566 12.1595C84.2908 12.1595 89.0764 17.1252 89.0764 23.2368C89.0764 29.3484 84.2908 34.314 78.3566 34.314Z"
                    fill="white" />
                <path
                    d="M10.5284 15.3426H16.0798V10.3133H10.5284V0H5.48753V10.3133H0V15.3426H5.48753V38.1338H5.67896H10.5284H21.7587V33.1045H10.5284V15.3426Z"
                    fill="white" />
            </svg>
        </a>
        <ul class="navigation-list un-active">
            <li class="nav-list-item">
                <a href="services.html">Услуги</a>
                <div class="dropdown-content-wrapper">
                    <div class="dropdown-content">
                        <ul class="dropdown-list">
                            <li class="dropdown-list-item dropdown-head">
                                <a href="" class="dropdown-link">Решения для развития бизнеса</a>
                            </li>
                            <li class="dropdown-list-item">
                                <a href="" class="dropdown-link">Оптимизация бизнес-процессов (BPM, BPR)</a>
                            </li>
                            <li class="dropdown-list-item">
                                <a href="" class="dropdown-link">Стратегическое развитие компании</a>
                            </li>
                            <li class="dropdown-list-item">
                                <a href="" class="dropdown-link">Интернет-маркетинг</a>
                            </li>
                        </ul>
                        <ul class="dropdown-list">
                            <li class="dropdown-list-item dropdown-head">
                                <a href="" class="dropdown-link">Трудовой аудит и консалтинг</a>
                            </li>
                            <li class="dropdown-list-item">
                                <a href="" class="dropdown-link">Консультации по трудовым вопросам</a>
                            </li>
                            <li class="dropdown-list-item">
                                <a href="" class="dropdown-link">Корпоративное обучение </a>
                            </li>
                            <li class="dropdown-list-item">
                                <a href="" class="dropdown-link">Кадровый аудит</a>
                            </li>
                            <li class="dropdown-list-item">
                                <a href="" class="dropdown-link">Трудовые споры и медиация </a>
                            </li>
                        </ul>
                        <ul class="dropdown-list">
                            <li class="dropdown-list-item dropdown-head">
                                <a href="" class="dropdown-link">Управление интеллектуальной собственностью</a>
                            </li>
                            <li class="dropdown-list-item">
                                <a href="" class="dropdown-link">Регистрация, депонирование и защита авторских прав</a>
                            </li>
                            <li class="dropdown-list-item">
                                <a href="" class="dropdown-link">Решения для СМИ, PR и маркетинга, маркетплейсов</a>
                            </li>
                            <li class="dropdown-list-item">
                                <a href="" class="dropdown-link">Аудит ОИС (IP audit)</a>
                            </li>
                            <li class="dropdown-list-item">
                                <a href="" class="dropdown-link">Регистрация СМИ и Интернет-СМИ </a>
                            </li>
                            <li class="dropdown-list-item">
                                <a href="" class="dropdown-link">Корпоративное обучение</a>
                            </li>
                        </ul>
                        <ul class="dropdown-list">
                            <li class="dropdown-list-item dropdown-head">
                                <a href="" class="dropdown-link">Юридическое обслуживание бизнеса</a>
                            </li>
                            <li class="dropdown-list-item">
                                <a href="" class="dropdown-link">Регистрация и перерегистрация юрлиц</a>
                            </li>
                            <li class="dropdown-list-item">
                                <a href="" class="dropdown-link">Разработка договорной базы</a>
                            </li>
                            <li class="dropdown-list-item">
                                <a href="" class="dropdown-link">Юридическое сопровождение сделок</a>
                            </li>
                            <li class="dropdown-list-item">
                                <a href="" class="dropdown-link">Претензионно-исковая работа</a>
                            </li>
                            <li class="dropdown-list-item">
                                <a href="" class="dropdown-link">Правовой аудит due diligence </a>
                            </li>
                        </ul>
                    </div>
                </div>
            </li>
            <li class="nav-list-item"><a href="">Проекты</a></li>
            <li class="nav-list-item"><a href="">Команда</a></li>
            <li class="nav-list-item"><a href="">Статьи</a></li>
            <li class="nav-list-item"><a href="">Контакты</a></li>
        </ul>
        <button class="burger" onclick="$('.burger').toggleClass('active un-active');"></button>
    </nav>
    <div class="container hero">
        <div class="hero-img">
            <img src="images/banner/advisor.png" alt="advisor" />
        </div>
        <div class="hero-txt">
            <p>
                Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do
                eiusmod tempor incididunt ut labore et dolore magna aliqua.
            </p>
            <a href="" class="more-btn"><span>Консультация</span><svg width="11" height="11" viewBox="0 0 11 11"
                                                                      fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M2 1H10M10 1V9M10 1L1 10" stroke="white" stroke-width="1.5" />
                </svg>
            </a>
        </div>
    </div>
</header>
